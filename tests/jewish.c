...
START_TEST(jewish_jdn)
{
    uint32_t jd = 0;
    uint32_t out_jd = 0;
    for (jd = 0; jd < 2488069; jd++)
    {
        test_julian_day(&jw_to_jdn, &jdn_to_jw, jd, &out_jd);
        assert(jd == out_jd);
    }
}
END_TEST

START_TEST(jewish_gregorian)
{
    uint32_t jd = 0;
    uint32_t out_jd = 0;
    uint32_t in_jd;
    ...
    for (jd = 0; jd < 2488069; jd++)
    {
        test_gregorian_calendar(&jw_to_gr, &gr_to_jw, jd,
            &gyi, &gmi, &gdi,
            &gyo, &gmo, &gdo);
        assert(gyi == gyo);
        assert(gmi == gmo);
...
Suite* create_tests(void)
{
    ...
    jewish = tcase_create("Jewish");
    tcase_add_test(jewish, jewish_jdn);
    tcase_add_test(jewish, jewish_gregorian);

    suite_add_tcase(suit, jewish);
    return suit;
}
