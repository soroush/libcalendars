...
LIBCALENDAR_API
uint8_t
jw_days_in_month (uint8_t month, int16_t year)
{
  switch (month)
    {
    case 1:
...
LIBCALENDAR_API
uint8_t
jw_is_valid (int16_t year, uint8_t month, uint16_t day)
{
...
v1 (int32_t x)
{
  ...
  return 32336 * qx + fdiv (15 * qx + 765433 * mod (c1, 1095) + 12084, 25920);
}
...
v2 (int32_t x)
{
  ...
  const int32_t qx = fdiv (c1, 1095);
  const int32_t v1x
      = 32336 * qx + fdiv (15 * qx + 765433 * mod (c1, 1095) + 12084, 25920);
...
c4 (int32_t x, int32_t y)
{
  ...
  const int32_t v1x = 32336 * qx + fdiv (15 * qx + 765433 * rx + 12084, 25920);
  return v1x + mod (fdiv (6 * mod (v1x, 7), 7), 2)
         + 2 * mod (fdiv (L2 (x) + 19, 15), 2)
         + mod (fdiv (L2 (x - 1) + 7, 15), 2) + fdiv (384 * y + 7, 13)
...
LIBCALENDAR_API
void
jw_to_jdn (uint32_t *jd, int16_t year, uint8_t month, uint16_t day)
{
  ...
  *jd = j0 - 177 + 32336 * qx + fdiv (15 * qx + 765433 * r (x1) + 12084, 25920)
        + mod (fdiv (6 * mod (v1 (x1), 7), 7), 2)
        + 2 * mod (fdiv (L2 (x1) + 19, 15), 2)
        + mod (fdiv (L2 (x1 - 1) + 7, 15), 2) + fdiv (384 * x3 + 7, 13)
        + mod (fdiv (L (x3) + 7, 2), 15) * fdiv (x3 + 4, 12)
        - mod (fdiv (385 - L (x3), 2), 15) * fdiv (x3 + 3, 12) + z4;
}

LIBCALENDAR_API
void
jdn_to_jw (uint32_t jd, int16_t *year, uint8_t *month, uint16_t *day)
{
  const int32_t y4 = jd - 347821;
  const int32_t q = fdiv (y4, 1447);
  const int32_t r = mod (y4, 1447);
  const int32_t y1_p = 49 * q + fdiv (23 * q + 25920 * r + 13835, 765433);
...
LIBCALENDAR_API
void
jw_to_gr (int16_t jyear, uint8_t jmonth, uint16_t jday, int16_t *gyear,
...
LIBCALENDAR_API
void
gr_to_jw (int16_t gyear, uint8_t gmonth, uint16_t gday, int16_t *jyear,
