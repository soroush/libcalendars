...
LIBCALENDAR_API
uint8_t
is_days_in_month (uint8_t month, int16_t year)
{
  if (year == 0)
    {
      return 0;
...
LIBCALENDAR_API
uint8_t
is_is_valid (int16_t year, uint8_t month, uint16_t day)
{
  if (year != 0 && month <= is_month_in_year (year)
      && day <= is_days_in_month (month, year))
    {
      return 1;
    }
  return 0;
...
LIBCALENDAR_API
void
is_to_jdn (uint32_t *jd, int16_t year, uint8_t month, uint16_t day)
{
  if (year <= 0)
    {
      ++year;
    }
  *jd = fdiv (10631 * year - 10617, 30) + fdiv (325 * month - 320, 11) + day
        + 1948439;
}

LIBCALENDAR_API
void
jdn_to_is (uint32_t jd, int16_t *year, uint8_t *month, uint16_t *day)
{
...
LIBCALENDAR_API
void
is_to_gr (int16_t jyear, uint8_t jmonth, uint16_t jday, int16_t *gyear,
...
LIBCALENDAR_API
void
gr_to_is (int16_t gyear, uint8_t gmonth, uint16_t gday, int16_t *jyear,
