...
LIBCALENDAR_API
uint8_t ba_is_leap(int16_t year)
{
...
LIBCALENDAR_API
uint8_t ba_days_in_month(uint8_t month, int16_t year)
{
    const int16_t cycle = mod(year, 19);
    uint8_t m0 = month;
...
LIBCALENDAR_API
uint16_t ba_days_in_year(int16_t year)
{
...
LIBCALENDAR_API
uint8_t ba_month_in_year(int16_t year)
{
    if (ba_is_leap(year) == 1)
        return 13;
    return 12;
...
LIBCALENDAR_API
uint8_t ba_is_valid(int16_t year, uint8_t month, uint16_t day)
{
...
LIBCALENDAR_API
void ba_to_jdn(uint32_t* jd, int16_t year, uint8_t month, uint16_t day)
{
    *jd = fdiv(6940 * (fdiv(235 * year + 13, 19) + month - 1), 235) + day + 1607174;
}

LIBCALENDAR_API
void jdn_to_ba(uint32_t jd, int16_t* year, uint8_t* month, uint16_t* day)
{
...
LIBCALENDAR_API
void ba_to_gr(int16_t jyear, uint8_t jmonth, uint16_t jday,
...
LIBCALENDAR_API
void gr_to_ba(int16_t gyear, uint8_t gmonth, uint16_t gday,
