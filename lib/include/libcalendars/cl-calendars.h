...
#define CAL_PERSIAN CAL_SOLAR_HIJRI
   ...
   */
  LIBCALENDAR_API uint8_t days_in_month (uint8_t calendar, uint8_t month,
                                         int16_t year);
   ...
   */
  LIBCALENDAR_API uint8_t is_valid (uint8_t calendar, int16_t year,
                                    uint8_t month, uint16_t day);

  /**
   * Converts a given date (year, month, day) to Julian Day Number.
   */
  LIBCALENDAR_API void to_jdn (uint8_t calendar, uint32_t *jd, int16_t year,
                               uint8_t month, uint16_t day);

  /**
   * Converts a Julian Day Number to date (year, month, day).
   */
  LIBCALENDAR_API void to_date (uint8_t calendar, uint32_t jd, int16_t *year,
                                uint8_t *month, uint16_t *day);
  ...
  LIBCALENDAR_API void convert_date (uint8_t icalendar, uint8_t ocalendar,
                                     int16_t iyear, uint8_t imonth,
                                     uint16_t iday, int16_t *oyear,
                                     uint8_t *omonth, uint16_t *oday);
